	...
	<view>
		...
		<view class="flex a-center" style="padding-right: 10rpx;">
			<view class="progress-container" id="container" ref="progressContainer" :style="{ background: inBgColor }">
				<view class="progress-content flex j-end" id="content" ref="progressContent"
					...
					<view class="textInside flex a-center j-center" v-if="textInside && !noData">
						<view style="text-align: center;">{{ percentage }}%</view>
					</view>
				</view>
				<view v-if="!isAnimate" class="progress-content flex j-end"
					style="background: linear-gradient(180deg, #FF6F5B 0%, #FFB8AE 100%);;"
			...
			<view>
				<view class="percentage" v-if="!textInside && !lineData && !noData && !isAnimate">{{ percentage }}%
				</view>
			</view>
		</view>
	</view>
...
<script>
	export default {
		...
		props: {
			...
			bgColor: {
				type: String,
				default: 'linear-gradient(180deg, #FF6F5B 0%, #FFB8AE 100%);'
			},
			// 是否不显示数据
			noData: {
				type: Boolean,
...
<style scoped lang="scss">
	...
	.j-between {
		...
		background: linear-gradient(180deg, #FF6F5B 0%, #FFB8AE 100%);
	}
